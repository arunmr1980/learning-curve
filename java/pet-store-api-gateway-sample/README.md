...
This compiles the client into a jar located at `./target/petstore-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'com.gc.petstore:petstore:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>com.gc.petstore</groupId>
...
This will compile the client and package all dependencies into a jar located at `./target/petstore-1.0-SNAPSHOT.jar`.
...
```java
petstore client = petstore.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`petstore`) is created for all actions (resource + method) in your API.
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of petstoreException.
```java
try {
    client.createPet(...);
} catch(petstoreException e) {
   // All service exceptions will extend from petstoreException.
   // Any unknown or unmodeled service exceptions will be represented as a petstoreException.
