...
This compiles the client into a jar located at `./target/ExamReportService-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'com.gc.examreport:ExamReportService:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>com.gc.examreport</groupId>
        <artifactId>ExamReportService</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/ExamReportService-1.0-SNAPSHOT.jar`.
...
```java
ExamReportServiceClientBuilder builder = ExamReportService.builder();
```
...
```java
ExamReportService client = ExamReportService.builder()
...
```java
ExamReportService client = ExamReportService.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`ExamReportService`) is created for all actions (resource + method) in your API.
...
```java
ExamReportService client = ExamReportService.builder().build();
GetExamReportExam_report_keyResult result = client.getExamReportExam_report_key(new GetExamReportExam_report_keyRequest());
```
...
```java
ExamReportService client = ExamReportService.builder().build();
client.getExamReportExam_report_key(new GetExamReportExam_report_keyRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of ExamReportServiceException.
```java
try {
    client.getExamReportExam_report_key(...);
} catch(ExamReportServiceException e) {
   // All service exceptions will extend from ExamReportServiceException.
   // Any unknown or unmodeled service exceptions will be represented as a ExamReportServiceException.
